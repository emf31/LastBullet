...
void Motor::GUI::init(const std::string& resourcesPath/*, irr::IrrlichtDevice *device*/){
	...
	m_context = &CEGUI::System::getSingleton().createGUIContext(GUIManager::i().getOpenGLRenderer()->getDefaultRenderTarget());
...
void Motor::GUI::draw() {
	...
	GUIManager::i().getOpenGLRenderer()->beginRendering();
	m_context->draw();
	GUIManager::i().getOpenGLRenderer()->endRendering();
