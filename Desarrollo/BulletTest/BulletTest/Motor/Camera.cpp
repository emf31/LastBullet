...
void Camera::update()
{
	if (m_entity != NULL) {
		setPosition(Vec3<float>(
			...
			m_entity->getRenderPosition().getY() + m_entity->getNode()->getScale().getY()+60,	//Situamos la camara en el top del player(el +50 es por el mesh que no situa la camara bien)
