...
void Camera::update()
{
	...
	if (m_entity != nullptr) {
		setPosition(Vec3<float>(
			...
			m_entity->getRenderPosition().getY() + m_entity->getNode()->getScale().getY()+5,	//Situamos la camara en el top del player(el +50 es por el mesh que no situa la camara bien)
